<template>
  <div id="app" class="app">
    ...
    <div class="container">
      <form class="main-form" @submit.prevent="generate">
        ...
        <div class="common-fields vue-ui-grid col-2 default-gap">
          <VueFormField :title="i18n('repo-title')" :subtitle="i18n('repo-subtitle')" class="first-row">
            <VueSelect v-model="repo">
              <VueSelectButton v-for="option of repos" :key="option.id" :value="option" :label="option.name" />
            </VueSelect>
          </VueFormField>

          <VueFormField :title="i18n('type-title')" class="first-row">
            <VueGroup v-model="type" class="extend">
              <VueGroupButton v-for="option of types" :key="option.id" :value="option.id">
                {{ option.name }}
              </VueGroupButton>
            </VueGroup>
          </VueFormField>
              ...
              <div class="similar-issues" v-if="issues.length">
                ...
                <ul>
                  <li v-for="issue in issues" :key="issue.id">
                    <a class="issue" :href="issue.html_url" target="_blank" rel="noreferrer" tabindex="-1">
                      {{ issue.title }}
                    </a>
                  </li>
                </ul>
                ...
                <p v-if="showIssueToggleControl">
                  <span v-if="!showingAllIssues" role="button" @click="showingAllIssues = true">
                    {{ i18n('show-more') }}
                  </span>
        ...
        <div class="form-actions">
          <VueButton type="submit" class="primary big" :label="i18n('preview')" />
        </div>
      </form>

      <VueModal v-if="show" :title="i18n('preview-title')" class="medium" @close="show = false">
        ...
        <div slot="footer" class="actions">
          <!-- <VueButton
            ...
            @click="create()"
          /> -->
          <el-button type="primary" @click="create()">{{ i18n('create') }}</el-button>
        </div>
      </VueModal>
    </div>
    ...
    <footer class="app-footer">
      Inspired by
      <a href="https://new-issue.vuejs.org/" target="_blank" rel="noopener noreferrer"> Vue Issue Helper </a
      >&nbsp;·&nbsp;
...
export default {
  ...
  methods: {
    ...
    create() {
      ...
      const label = this.type === 'feature-request' ? '&labels=help%20wanted' : '';
