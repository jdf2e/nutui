<template>
  <div class="bug-report" style="margin: 0">
    <div class="vue-ui-grid col-2 default-gap">
      <VueFormField
        :title="repo.name === 'nutui-vue' ? i18n('nutui-repo-scenes-vue') : i18n('nutui-repo-scenes-react')"
      >
        <VueSelect v-model="attrs.scenes" :label="attrs.scenes.name" :placeholder="i18n('nutui-repo-placeholder')">
          <VueSelectButton v-for="option of nutuiScenes" :key="option.id" :value="option" :label="option.name">
          </VueSelectButton>
        </VueSelect>
      </VueFormField>
      <VueFormField :title="i18n('version-title-vue')" v-if="repo.name === 'nutui-vue'">
        <VueTypeAhead
    ...
    <div :class="['vue-ui-grid', 'default-gap', attrs.scenes.id != 'h5-env' ? 'col-2' : '']">
      <VueFormField :title="i18n('nutui-repo-vue')" v-if="repo.name == 'nutui-vue'">
        <VueTypeAhead
      ...
      <VueFormField :title="i18n('nutui-repo-execution')" v-if="attrs.scenes.id != 'h5-env'">
        <VueSelect
          v-model="attrs.runEnvVal"
          :label="attrs.runEnvVal.name"
          :placeholder="i18n('nutui-repo-placeholder')"
        >
          <VueSelectButton v-for="option of runEnv" :key="option.id" :value="option" :label="option.name">
          </VueSelectButton>
        </VueSelect>
      </VueFormField>
    </div>
    ...
    <div class="vue-ui-grid col-2 default-gap">
      <VueFormField :title="i18n('nutui-repo-use')">
        <VueSelect v-model="attrs.useWay" :label="attrs.useWay.name" :placeholder="i18n('nutui-repo-placeholder')">
          <VueSelectButton v-for="option of wayList" :key="option.id" :value="option" :label="option.name">
          </VueSelectButton>
        </VueSelect>
      </VueFormField>

      <VueFormField :title="i18n('nutui-repo-node')">
        <VueInput v-model="attrs.nodejs" required autofocus placeholder="16.18.0" />
      </VueFormField>
    </div>

    <div class="vue-ui-grid col-2 default-gap">
      <VueFormField :title="i18n('nutui-repo-boswer')">
        <VueInput v-model="attrs.boswer" required autofocus :placeholder="i18n('nutui-repo-boswer-placeholder')" />
      </VueFormField>

      <VueFormField :title="i18n('nutui-repo-system')">
        <VueInput v-model="attrs.system" required autofocus :placeholder="i18n('nutui-repo-system-placeholder')" />
      </VueFormField>
    </div>

    <VueFormField class="span-2" :title="i18n('nutui-repo-taro-info')" v-if="attrs.scenes.id != 'h5-env'">
    ...
    <VueFormField :title="i18n('repro-title')">
      <VueInput type="url" v-model="attrs.reproduction" :disabled="isCLI && reproNotAvailable" required />
    ...
    <VueFormField class="span-2 span-or" :title="i18n('extra-title')" :subtitle="i18n('extra-subtitle')">
...
export default {
  ...
  computed: {
    suggestions() {
      return this.versions.slice().sort((a, b) => (gt(a.value, b.value) ? -1 : 1));
    },
    suggestionsVue() {
      return this.versionsVue.slice().sort((a, b) => (gt(a.value, b.value) ? -1 : 1));
    },

    suggestionsReact() {
      return this.versionsReact.slice().sort((a, b) => (gt(a.value, b.value) ? -1 : 1));
    },
  ...
  methods: {
    setScenes() {
      this.nutuiScenes = this.repo.name == 'nutui-vue' ? this.nutuiScenesVue : this.nutuiScenesReact;
    ...
    async fetchReactVersions() {
      ...
      const result = await response.json();
      const versionsList = await Object.keys(result.versions);
      const releases = await versionsList.filter((item) => item.indexOf('beta') == -1 && item.indexOf('-') == -1);
