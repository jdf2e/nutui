...
```vue
<template>
  <nut-image-preview :show="state.showPreview" :images="state.imgData" @close="hideFn" @long-press="longPress" />
  <nut-cell is-link title="长按图片事件，保存到相册" :show-icon="true" @click="showFn"></nut-cell>
</template>
...
| long-press`4.2.1` | 小程序长按图片触发的事件 | (image: { src: string }) => void |
