...
import { mockScrollTop, sleep } from './../../../utils/unit';
...
test('Menu: open & close events & onClickAway', async () => {
  const open = vitest.fn();
  const close = vitest.fn();
  const wrapper = mount(() => {
    return (
      <div>
        <div id="test" style="height: 20px;">
          placeholder
        </div>
        <Menu>
          <MenuItem modelValue={1} onOpen={open} onClose={close} options={options1} />
          <MenuItem modelValue="a" options={options2} />
        </Menu>
      </div>
    );
  });
  ...
  await nextTick();
  const items = wrapper.findAll('.nut-menu-item');
  const titles = wrapper.findAll('.nut-menu__title');
  expect(items.length).toBe(2);
  expect(items[0].attributes('style')).contain('display: none');
