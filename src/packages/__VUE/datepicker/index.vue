...
export default create({
  ...
  setup(props, { emit }) {
    ...
    const ranges = computed(() => {
      const { maxYear, maxDay, maxMonth, maxHour, maxMinute, maxSeconds, maxQuarter } = getBoundary(
        'max',
        state.currentDate
      );

      const { minYear, minDay, minMonth, minHour, minMinute, minSeconds, minQuarter } = getBoundary(
        'min',
        state.currentDate
      );
