...
const confirm = ({ selectedValue, selectedOptions }) => {
  showToast.text(selectedOptions.map((val) => val.text).join(''));
};
</script>
```
...
| type | 时间类型，可选值 `date`(年月日) `time`(时分秒) `year-month`(年月) `month-day`(月日) `datehour`(年月日时) `hour-minute`(时分`v4.0.5`)`datetime`(年月日时分)`dateTime`(年月日时分秒) | string | `date` |
