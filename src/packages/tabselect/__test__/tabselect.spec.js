...
describe('TabSelect.vue', () => {
  ...
  it('mainTitle标题', () => {
    return Vue.nextTick().then(function () {
      expect(wrapper.findAll('.nut-tabselect-main-title').at(0).text()).toBe('配送测试');
    });
  });

  it('subTitle标题', () => {
    return Vue.nextTick().then(function () {
      expect(wrapper.findAll('.nut-tabselect-sub-title').at(0).text()).toBe('送达时间测试');
  ...
  it('defaultContent默认内容', () => {
    return Vue.nextTick().then(function () {
      expect(wrapper.findAll('.nut-tab-panel-list').at(0).text()).toBe('11:00——12:00');
    });
  });
  ...
  it('设置max', () => {
    ...
    wrapper.findAll('.nut-tab-panel-list').at(4).trigger('click');
    return Vue.nextTick().then(function () {
      expect(wrapper.findAll('.nut-tab-panel-list-active').length).toBe(2);
    });
  });
  ...
  it('tabList整体数据', () => {
    return Vue.nextTick().then(function () {
      expect(wrapper.findAll('.nut-title-nav').at(0).text()).toBe('京东快递测试');
    });
  });
  ...
  it('设置show', () => {
    ...
    return Vue.nextTick().then(function () {
      expect(wrapper.findAll('.popup-box').at(0).attributes('display')).toBe(undefined);
    });
  });
